...
int TripToNode(int nx, int ny, int x, int y, int d) {
    return nx*ny*d+ny*x+y;
}


void neighbors(int nx, int ny, int x, int y,
               vector<int>& x_n, vector<int>& y_n, int& n) {
    // Corner --> 2 neighbors
    if (x==0 && y==0) {
        n = 2;
        x_n[0] = +1; y_n[0] = 0;
...
void build_graph(Graph<int,int,int>& G,
    ...
    for (int x=0; x<nx; x++) {
        for (int y=0; y<ny; y++) {
            ...
            for (int d=0; d<nd; d++) {
                // Corresponding node
                int node_id = TripToNode(nx, ny, x, y, d);

                int numNeighbors;
                vector<int> xNeighbors(4);
                vector<int> yNeighbors(4);
                neighbors(nx, ny, x, y, xNeighbors, yNeighbors, numNeighbors);
                float Kp = 1 + (nd - 1) * numNeighbors * lambda;

                // Compute weight
                float wp = (i+dmin+d+win < I2.width()) ? (wcc * rho(zncc(I1, I1M, I2, I2M, i, j, i+dmin+d, j)) + Kp) : INF;
                assert(wp >= 0);

                // Add edge from source to first layer
                if (d==0) {
                    G.add_tweights(node_id, /*weights*/ wp, 0);
                }
                // Add edge from last layer to sink
                // Add edge to the previous layer
                else if (d==nd-1) {
                    int wpp = (i+dmax+win < I2.width()) ? (wcc * rho(zncc(I1, I1M, I2, I2M, i, j, i+dmax, j)) + Kp) : INF;
                    assert(wpp >= 0);
                    G.add_tweights(node_id, /*weights*/ 0, wpp);

                    int node_prev = TripToNode(nx, ny, x, y, d-1);
                    G.add_edge(node_prev, node_id, /*weights*/ wp, 0);
                }
                // Add edge to previous layer
                else {
                   int node_prev = TripToNode(nx, ny, x, y, d-1);
                   G.add_edge(node_prev, node_id, /*weights*/ wp, 0);
                }

                // Add edge to neighbors (once)
                for (int id=0; id<numNeighbors; id++) {
                    int node_n = TripToNode(nx, ny, x + xNeighbors[id], y + yNeighbors[id], d);
                    if (node_id > node_n) G.add_edge(node_id, node_n, /*weights*/ lambda, lambda);
                }
            }
        }
    }
    ...
    for (int x=0; x<nx; x++) {
        for (int y=0; y<ny; y++) {
            ...
            while (G.what_segment(node_id) == Graph<int,int,int>::SOURCE && d<nd-1) {
                d++;
                node_id = TripToNode(nx, ny, x, y, d);
            }
            if (d == nd-1) {
                D(x,y) = nd + dmin;
            } else {
