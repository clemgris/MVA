...
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
    ...
    for (int k = - win; k <= win; k++) {
        for (int l = - win; l <= win; l++) {
            float I1 = (im1(i1 + k, j1 + l) - m1);
            float I2 = (im2(i2 + k, j2 + l) - m2);
            correlation += I1 * I2;
            normI1 += I1 * I1;
            normI2 += I2 * I2;
        }
    }
    dist = correlation / (sqrt(normI1) * sqrt(normI2) + EPS);
    return dist;
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            int bestDisp;
            float NCC;
            // Ignore disparities for which the matching patch is out of im2
            for (int d = max(dmin, win-x); d <= min(dmax, im2.width()- (1+x+win)); d++) {
                // Compute NCC on patches center on (x, y) in im1 and (x+d, y) in im2
            ...
            // Update queue and disparity map
            if (bestNCC >= nccSeed) {
                seeds(x, y) = true;
                disp(x, y) = bestDisp;
        ...
        for(int i=0; i<4; i++) {
               ...
               ! seeds(x,y))
            {
                ...
                for (int d = max(dmin, s.d - 1); d <= min(s.d + 1, dmax) ; d++) {
                    // Ignore disparity if the matching patch is out of im2
